...
before_last_quarter_start <-
  dates %>%
  filter(date != snapshot_date) %>%
  mutate(month = month(date)) %>%
  filter(month %in% c(3, 6, 9, 12)) %>%
  select(date) %>% tail(3) %>% head(1) %>% pull()
...
#     #datums naar ultimo's
     mutate(date = floor_date(date, unit = "month"),
            date = lubridate::rollback(date + months(1))) |> 
     left_join(holdings, by = c("Naam" = "name"))
...
naam_isin <- readxl::read_excel(holdings_file, sheet = "naam_isin", 
            ...
            #na import geen join ivm de spaces
             mutate(Naam_VOLK = str_squish(Naam_VOLK),
                    Naam_new_sheet = str_squish(Naam_new_sheet))
...
portfolios_tabel_koersen_AIRS <- 
  portfolio_downloads_ruw %>%
                mutate(portfolio_date = str_remove(portfolio_date, ".xlsx"),
                       ...
                       # Naam = ifelse(Naam == "Vanguard FTSE Emerg. Markets ETF",
                       #               "Vanguard FTSE Emerging Mkts Index Fd", Naam),
...
```{r portfolios_tabel_koersen_AIRS_maken_new}
...
#   portfolios_tabel |> ungroup() |> left_join(naam_isin |> select(-Naam_VOLK) |> unique(), by = c("name" = "Naam_new_sheet"))
# 
# values_portfolios_tabel_ISIN <-
#   portfolios_tabel_ISIN |>
#   group_by(date, portfolio) |>
#   summarize(value_eur = sum(value_eur), .groups = "drop")
# values_portfolio_downloads_ruw_new_cutoffs <-
#   portfolio_downloads_ruw_new_cutoffs |>
#   group_by(date, portfolio) |>
#   summarize(value_eur = sum(Marktwaarde), .groups = "drop")
# 
# values_verschillen <- 
#   portfolio_downloads_ruw_new_cutoffs |> rename(new_value = Marktwaarde) |> group_by(portfolio, date) |> summarise(new_value = sum(new_value)) |> 
#     left_join(portfolios_tabel_ISIN |> rename(old_value = value_eur) |> group_by(portfolio, date) |> summarise(old_value = sum(old_value)), by = c("date", "portfolio")) |> mutate(diff = new_value - old_value, abs_diff_pct = abs(round(diff/old_value, 3))) |> arrange(desc(abs_diff_pct))
# 
# values_verschillen_regels <- portfolio_downloads_ruw_new_cutoffs |> rename(new_value = Marktwaarde) |> left_join(portfolios_tabel_ISIN, by = c("date", "portfolio", "ISIN")) |> rename(old_value = value_eur) |> mutate(diff = new_value - old_value, abs_diff_pct = abs(round(diff/old_value, 3))) |> arrange(desc(abs_diff_pct))
# 
# #ok check
# aantal_regels__verschillen <- portfolios_tabel_ISIN |> ungroup() |> group_by(date, portfolio) |> summarize(n_old = n(), .groups = "drop") |> full_join(portfolio_downloads_ruw_new_cutoffs |> group_by(date, portfolio) |> summarize(n_new = n(), .groups = "drop"), by = c("date", "portfolio")) |> mutate(abs_diff_n = abs(n_new - n_old)) |> arrange(desc(abs_diff_n))
# 
# shares_verschillen <- portfolio_downloads_ruw_new_cutoffs |> left_join(portfolios_tabel_ISIN, by = c("date", "portfolio", "ISIN"))|> select(date, portfolio, Naam, ISIN, old_shares = shares, new_shares = Aantal) |> mutate(diff_shares = abs(new_shares - old_shares), abs_diff_shares_pct = abs(round(diff_shares/old_shares, 3))) |> 
#     arrange(desc(abs_diff_shares_pct))
# 
# price_verschillen <- portfolio_downloads_ruw_new_cutoffs |> left_join(portfolios_tabel_ISIN, by = c("date", "portfolio", "ISIN"))|> select(date, portfolio, Naam, ISIN, old_price = price, new_price = Koers) |> mutate(diff_price = abs(new_price - old_price)) |> mutate(ratio = new_price/old_price) |> arrange(desc(diff_price)) 
# 
...
if(bij_pp) {
  ...
  portfolios_tabel_lastdate |> write_rds("/Users/pieter/Library/CloudStorage/OneDrive-Personal/R Projects/Beleggingsvoorstel/shiny/portfolios_tabel_lastdate.rds")
  portfolios_tabel_lastdate |> write_rds('/Users/pieter/Library/CloudStorage/OneDrive-Personal/R Projects/Vergelijkingssites/portfolios_tabel_lastdate.rds')
  }
```
...
```{r attr_pfs_llq}
attr_tabel_RDRM_simple_summaries_nice(start_date = before_last_quarter_start, end_date = last_quarter_start)
```
...
```{r attr_RDRM_graphs_llq}

chart_attr_effecten_RDRM(start_date = before_last_quarter_start, end_date = last_quarter_start) 
```
...
```{r attr_tabel_individuele_tickers_sel_ff}
#mee
attr_tabel_individuele_tickers_sel_ff <- function(portfolio = "VP100", start_date = last_ultimo_year, end_date = snapshot_date) {
  ...
  mutate(name = ifelse(is.na(sector), "", name),
         ...
         trr = ifelse(sector == "Totaal", 
                      attr_tabellen_RDRM_simple |> 
                      filter(portfolio == !!portfolio, date > start_date, date <= end_date) |> 
                      summarise(trr = prod(1 + pf_rt_RD) - 1) |> 
                      pull(trr), 
                      trr)
         ) |> 
...
```{r attr_tabel_individuele_tickers_sel_ff_nice_functie}

attr_tabel_individuele_tickers_sel_ff_nice <- function(portfolio = "VP100", start_date = last_ultimo_year, end_date = snapshot_date) {
  tabel <- attr_tabel_individuele_tickers_sel_ff(portfolio = portfolio, start_date = start_date, end_date = end_date)
  tabel_nice <-
      tabel |>       
      #valuta en koersen erbij op beide momenten
      left_join(currencies, by = "ticker") |>
      left_join(all_prices_local_long |> filter(date == start_date), by = c("ticker")) |> select(-date) |> 
      left_join(all_prices_local_long |> filter(date == end_date), by = c("ticker")) |> select(-date) |> 
      select(sector, ticker, name, CRNCY, PX_LAST.x, start_weight, wt_bm_start, OU_start, PX_LAST.y, end_weight, wt_bm_end, OU_end, trr, sel_ff) |> 
      rename(ccy = CRNCY, price = PX_LAST.x, `price ` = PX_LAST.y, wt_in_RD = start_weight, wt_in_bm = wt_bm_start, OU = OU_start,
             `wt_in_RD ` = end_weight, `wt_in_bm ` = wt_bm_end, `OU ` = OU_end, trr_eur = trr) |> 
      gt(id = "one") |>
      fmt_percent(columns = contains(c("wt", "OU", "trr", "sel_ff")), decimals = 2) |> 
      fmt_number(columns = contains("price"), decimals = 2) |>
      tab_spanner(columns = c(price, wt_in_RD, wt_in_bm, OU), label = format(start_date, "%d-%b-%y")) |>
      tab_spanner(columns = c(`price `, `wt_in_RD `, `wt_in_bm `, `OU `), label = format(end_date, "%d-%b-%y")) |>
      tab_spanner(columns = c(trr_eur, sel_ff), label = "Performance") |>
      tab_style(locations = cells_body(columns = c(OU, `OU `, sel_ff)),
                style = list(cell_text(weight = "bold"))) |>
      gt_add_dividers_right(columns = c(4, 8, 12)) |>
      tab_style(style = cell_borders(sides = "bottom", weight = px(1)), locations = cells_body(rows = nrow(tabel) - 3)) |> 
      tab_style(style = cell_borders(sides = "bottom", weight = px(1)), locations = cells_body(rows = nrow(tabel) - 1)) |> 
      gt_data_colors(columns = contains(c("OU", "sel_ff"))) |>
      gt_delete_first_col_label() |>
      gt_cols_align() |>
      gt_first_col_bold() |>
      ...
      gt_font_padding_align() |>
      gt_last_row_bold() |>
      tab_options(table.font.size = "10px") |>  # Adjust as needed (e.g., "small", "16px", "1.2em")
      #gt_container_scroll() |>
...
```{r barchart_bijdrage_aan_trr_per_regel_vps_llq}
#| fig-height: 7
#| fig-width: 10

attr_tabel_individuele_tickers_sel_ff_nice(start_date = before_last_quarter_start, end_date = last_quarter_start)
```
